...
// Full scrollable list + no filters + per-article audio 😮
// ========================================================

// ---------- CONFIG ----------
const MAX_ITEMS_WIDGET = 8;   // number of headlines to show in widget
const MAX_ITEMS_MENU = 25;    // number of headlines in full-screen menu
const SUMMARY_SENTENCES = 5;  // heuristic AI summary (~5 sentences)
const SPEAK_HEADLINES = true; // spoken briefing on menu launch
const SHOW_KEYWORD_TRENDS = true; // show top keywords in widget

// ---------- PROFILES ----------
const PROFILES = [
  { name: "Tech", keywords: [], emoji: "🖥" },       // no filter
...
function filterResults(titles, links, descriptions, keywords, maxItems) {
  ...
  if(results.length===0) results.push({title:"No headlines available", link:"https://news.google.com", summary:"Try again later."});
  return results;
}
...
if(isWidget){
  ...
  widget.url = "scriptable:///run/GoogleNewsUltimate"; // tap widget → full menu
  Script.setWidget(widget);
  Script.complete();
}

// ---------- FULL SCREEN MENU ----------
const rssFull = await fetchRSS(CATEGORIES[0].url);          // fetch RSS
const parsedFull = parseRSS(rssFull);                       // parse RSS correctly
const resultsFull = filterResults(
  parsedFull.titles,
  parsedFull.links,
  parsedFull.descriptions,
